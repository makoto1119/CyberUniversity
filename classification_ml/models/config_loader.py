...
class ConfigLoader:
    def __init__(self, config_path: str = "model_config.json"):
        ...
        preprocess_config_path = os.path.join(os.path.dirname(current_dir), 'preprocess_nlp', 'nlp_config.json')
        ...
        try:
            with open(preprocess_config_path, 'r', encoding='utf-8') as f:
                ...
                normalize_enabled = self.get_normalize_enabled()
                ...
                print(f"ゆらぎ補正: {'有効' if normalize_enabled else '無効'}")
        except FileNotFoundError:
            print(f"警告: 前処理設定ファイルが見つかりません: {preprocess_config_path}")
            self.preprocess_config = {}
            print("デフォルト設定を使用します: stopwords=False, ゆらぎ補正=False")
